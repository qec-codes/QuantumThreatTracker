...
class SampleAlgorithm(QuantumAlgorithm):
    ...
    def __init__(
        self, crypt_params: CryptParams, alg_params: AlgParams | None = None
    ) -> None:
        super().__init__(crypt_params, alg_params)

    @staticmethod
    def get_algorithm_summary(alg_params: AlgParams | None = None) -> AlgorithmSummary:
        """
        Return a sample algorithm summary.

        Returns
        -------
            AlgorithmSummary: A sample algorithm summary.
        """
...
class TestGidneyEkeraOptimizer:
    ...
    def setup_method(self) -> None:
        ...
        self.params_small_windows = GidneyEkeraParams(
            num_exp_qubits=1536, window_size_exp=1, window_size_mul=1
        )
        self.params_medium_windows = GidneyEkeraParams(
            num_exp_qubits=1536, window_size_exp=4, window_size_mul=4
        )
        self.params_large_windows = GidneyEkeraParams(
            num_exp_qubits=1536, window_size_exp=10, window_size_mul=10
        )

    def test_optimize_gidney_ekera_qubit_count(self) -> None:
    ...
    def test_single_element_search_space(self) -> None:
        ...
        single_param = GidneyEkeraParams(
            num_exp_qubits=1536, window_size_exp=3, window_size_mul=3
        )
    ...
    def test_gidney_ekera_generate_search_space(self) -> None:
